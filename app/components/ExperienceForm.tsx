...
const ExperienceForm: React.FC<ExperienceFormProps> = ({ 
  ...
  const [hasNoExperience, setHasNoExperience] = useState(initialHasNoExperience); // Track if user has no experience
  ...
  const validateExperiences = (exps: Experience[]): boolean => {
    ...
    for (const exp of exps) {
      ...
      if (!exp.isCurrent && exp.startMonth && exp.startYear && exp.endMonth && exp.endYear) {
        const startDate = new Date(parseInt(exp.startYear), parseInt(exp.startMonth) - 1);
        const endDate = new Date(parseInt(exp.endYear), parseInt(exp.endMonth) - 1);
  ...
  }, [experiences, hasNoExperience, onExperiencesChange, onValidationChange, onHasNoExperienceChange]);
  ...
  const removeExperience = (id: string) => {
    if (experiences.length === 1) {
      Alert.alert(t(`${TRANSLATION_KEY}.cannot_remove`), t(`${TRANSLATION_KEY}.min_experience_required`));
      return;
    }
  ...
  const updateExperience = (id: string, field: keyof Experience, value: string | boolean) => {
    setExperiences(experiences.map(exp => {
      ...
      if (['startMonth', 'startYear', 'endMonth', 'endYear'].includes(field as string) || field === 'isCurrent') {
        const { startMonth, startYear, endMonth, endYear, isCurrent } = updatedExp;
  ...
  return (
    <VStack space="xl" style={styles.content} onTouchStart={handleOutsidePress}>
      <Heading size="xl" style={styles.heading}>{t(`${TRANSLATION_KEY}.experience_details`, 'Work Experience')}</Heading>
      
      <Text style={styles.encouragementText}>
        {t(`${TRANSLATION_KEY}.experience_encouragement`, 'Tell us about your work experience to help us find the best job matches for you.')}
      </Text>
      ...
      <FormControl size="md" style={styles.noExperienceContainer}>
        <HStack space="sm">
          <Pressable 
            style={[
              styles.checkbox, 
              hasNoExperience && styles.checkboxChecked
            ]}
          ...
          <Text style={styles.noExperienceText}>{t(`${TRANSLATION_KEY}.no_experience`, "I don't have any work experience")}</Text>
        </HStack>
      </FormControl>
      ...
      {!hasNoExperience && experiences.map((experience, index) => (
        <Box key={experience.id} style={styles.experienceCard}>
          <HStack style={styles.experienceHeader}>
            <Text style={styles.experienceTitle}>{t(`${TRANSLATION_KEY}.experience_number`, { number: index + 1 })}</Text>
          ...
          <FormControl size="md" style={styles.formField}>
            <FormControlLabel>
              <FormControlLabelText>{t(`${TRANSLATION_KEY}.company_name`)}</FormControlLabelText>
            </FormControlLabel>
            <Input>
              <InputField
                placeholder={t(`${TRANSLATION_KEY}.company_name_placeholder`)}
                value={experience.company}
                onChangeText={(text) => updateExperience(experience.id, 'company', text)}
              />
            </Input>
          </FormControl>
          ...
          <FormControl size="md" style={styles.formField}>
            <FormControlLabel>
              <FormControlLabelText>{t(`${TRANSLATION_KEY}.company_address`)}</FormControlLabelText>
            </FormControlLabel>
            <Input>
              <InputField
                placeholder={t(`${TRANSLATION_KEY}.company_address_placeholder`)}
                value={experience.companyAddress}
                onChangeText={(text) => updateExperience(experience.id, 'companyAddress', text)}
              />
            </Input>
          </FormControl>
          ...
          <FormControl size="md" style={styles.formField}>
            <FormControlLabel>
              <FormControlLabelText>{t(`${TRANSLATION_KEY}.city_area`)}</FormControlLabelText>
            </FormControlLabel>
            <HStack space="md">
              <Input style={{ flex: 1 }}>
                <InputField
                  ...
                  onChangeText={(text) => updateExperience(experience.id, 'companyCity', text)}
                />
              </Input>
              <Input style={{ flex: 1 }}>
                <InputField
                  ...
                  onChangeText={(text) => updateExperience(experience.id, 'companyArea', text)}
                />
              </Input>
            </HStack>
          </FormControl>
          ...
          <FormControl size="md" style={styles.formField}>
            <FormControlLabel>
              <FormControlLabelText>{t(`${TRANSLATION_KEY}.job_title`)}</FormControlLabelText>
            </FormControlLabel>
            <Input>
              <InputField
                placeholder={t(`${TRANSLATION_KEY}.job_title_placeholder`)}
                value={experience.jobTitle}
                onChangeText={(text) => handleJobTitleInputChange(experience.id, text)}
              />
            </Input>
            
            {/* Job Title Suggestions */}
            {showJobTitleOptions && selectedExperienceId === experience.id && (
                  ...
                  {filteredJobTitles.length > 0 ? (
                    filteredJobTitles.map((option) => (
                      <Pressable
                        ...
                        onPress={() => selectJobTitle(experience.id, option.value)}
                      >
                        <Text style={styles.suggestionText}>{option.label}</Text>
                      </Pressable>
                    ))
                  ) : (
                    <Text style={styles.noResultsText}>{t(`${TRANSLATION_KEY}.no_matching_jobs`)}</Text>
                  )}
                </ScrollView>
              </Box>
            )}
          ...
          <FormControl size="md" style={styles.formField}>
            <FormControlLabel>
              <FormControlLabelText>{t(`${TRANSLATION_KEY}.start_date`)}</FormControlLabelText>
            </FormControlLabel>
            <HStack space="md">
                ...
                <InputField
                  ...
                  onChangeText={(text) => {
                    ...
                    if (numericValue === '') {
                      ...
                      const month = Math.min(Math.max(parseInt(numericValue), 1), 12).toString();
                ...
                <InputField
                  ...
                  onChangeText={(text) => {
                    ...
                    if (numericValue.length <= 4) {
                      updateExperience(experience.id, 'startYear', numericValue);
                    }
                  }}
                  keyboardType="numeric"
                  maxLength={4}
          ...
          <FormControl size="md" style={styles.formField}>
            <HStack space="sm" >
              <Pressable 
                ...
                onPress={() => updateExperience(experience.id, 'isCurrent', !experience.isCurrent)}
              >
          ...
          {!experience.isCurrent && (
            <FormControl size="md" style={styles.formField}>
              <FormControlLabel>
                <FormControlLabelText>{t(`${TRANSLATION_KEY}.end_date`)}</FormControlLabelText>
              </FormControlLabel>
              <HStack space="md">
                <Input style={styles.durationInput}>
                  <InputField
                ...
                <Input style={styles.durationInput}>
                  <InputField
                    ...
                    onChangeText={(text) => {
                      ...
                      const numericValue = text.replace(/[^0-9]/g, '');
                      if (numericValue.length <= 4) {
                        updateExperience(experience.id, 'endYear', numericValue);
                      }
                    }}
                    keyboardType="numeric"
                    maxLength={4}
          ...
          {((experience.startMonth && experience.startYear) && 
            (experience.isCurrent || (experience.endMonth && experience.endYear))) && (
            <Box style={styles.durationDisplay}>
              <Text style={styles.durationText}>
                {t(`${TRANSLATION_KEY}.duration`, {
                  years: experience.years || '0',
                  yearPlural: experience.years !== '1' ? 's' : '',
                  months: experience.months && experience.months !== '0' ? experience.months : '',
